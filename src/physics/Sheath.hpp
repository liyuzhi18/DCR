...
inline base::Real calculate_Bohm_speed(
    ...
    const base::Real Ti_erg = Ti_eV * eV_to_erg;
    const base::Real mass_g = mass_amu * amu_g;

    const base::Real u_bohm = std::sqrt(std::max(0.0, Te_erg + gamma_i * Ti_erg) / mass_g);
    return u_bohm;
...
inline base::Real calculate_thermal_speed(
    ...
    const base::Real v_th = std::sqrt(8.0 * T_erg / (pi * mass_g));
    return v_th;
}
...
// sheath_potential_drop is interpreted as a multiple of Te.

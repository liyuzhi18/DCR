...
ComparisonRow run_case(const dcr::io::Config& cfg,
    ...
    auto cfg_run = cfg;
    cfg_run.plasma.eedf.type = (label == "maxwellian") ? "maxwellian" : "generalized_power";
    cfg_run.plasma.eedf.power_p = power_p;
...
void write_csv(const fs::path& output_path,
    ...
    out << "label,power_p,scd_cm3_s,acd_cm3_s,scd_ratio_to_maxwellian,acd_ratio_to_maxwellian,h_cm3,hplus_cm3\n";
...
int main(int argc, char** argv) {
    ...
    const std::vector<ComparisonRow> rows{
        run_case(cfg, atomic_data, 1.0, "maxwellian"),
        run_case(cfg, atomic_data, 0.7, "generalized_power_p0.7"),
        run_case(cfg, atomic_data, 0.8, "generalized_power_p0.8"),
        run_case(cfg, atomic_data, 0.9, "generalized_power_p0.9"),
        run_case(cfg, atomic_data, 2.0, "generalized_power_p2.0")
    };
    ...
    std::cout << "label,power_p,scd_cm3_s,acd_cm3_s,scd_ratio_to_maxwellian,acd_ratio_to_maxwellian,h_cm3,hplus_cm3\n";
    for (const auto& row : rows) {
        std::cout << row.label << ','
                  << row.power_p << ','
                  << row.scd_cm3_s << ','
